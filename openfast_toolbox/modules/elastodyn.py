...
def rotor_disk_points(ED, nP=10, origin=(0,0,0)):
    """ Points of the rotor disk, taken from wiz
    Needs further improvements, e.g. for Ptfm motion
    """
    ED = getEDClass(ED) # if ED is a filename

    # TODO Ptfm..

    tilt   = ED['ShftTilt']*np.pi/180
    yaw    = (ED['NacYaw'])*np.pi/180
    hT     = ED['TowerHt'] + ED['Twr2Shft']
    t2h    = ED['OverHang'] * np.sin(tilt)
    dX_hub = ED['OverHang']*np.cos(yaw)
    dY_hub = ED['OverHang']*np.sin(yaw)
    R      = ED['TipRad']*np.cos(ED['PreCone(1)']*np.pi/180)

    origin = np.asarray(origin).reshape(3,1)
    r_hub0 = np.asarray([ED['OverHang'], 0, hT+t2h]).reshape(3,1)  + origin
    r_TT   = np.asarray([0             , 0, hT]).reshape(3,1)      + origin

    e_vert_g   = np.array([0           ,0,   1]        ).reshape(3,1)
    e_shaft_g0 = np.array([np.cos(tilt),0,np.sin(tilt)]).reshape(3,1)
    T_wt2g     = RotMat_AxisAngle(e_vert_g, yaw)
    # Rotating the shaft vector so that its coordinate follow the new yaw position
    e_shaft_g = np.dot(T_wt2g , e_shaft_g0)
    r_hub = r_TT + np.dot(T_wt2g , r_hub0-r_TT)

    #e_shaft_g = e_shaft_g0
    #r_hub =r_hub0

    e_r = R*orth_vect(e_shaft_g)

    points=np.zeros((3,nP))
    theta=np.linspace(0,2*np.pi,nP) # dTheta=2 pi /(np-1)
    for i,t in enumerate(theta):
        T=RotMat_AxisAngle(e_shaft_g, t)
        points[:,i]= r_hub.ravel()+np.dot(T,e_r).ravel()
    return r_hub.flatten(), points
